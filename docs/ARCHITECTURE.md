...
1. **Event drain.** `pygame.event.get()` is processed and dispatched. Mouse and keyboard events flow first to any active dialog (modal), then to the top menu bar, then to the workspace controller, then to component-level handlers. Right-click is a delete dispatch — it walks the workspace looking for a hit on a component, wire, or text box.
2. **Workspace interaction update.** `WorkspaceInteractionController` advances drags and marquee selection.
...
4. **Render.** Layers are painted bottom-up: workspace background → wires → components → text boxes → component bank (toolbox) → top menu bar → any active dialog → CRT overlay.
...
`TopMenuBar` owns the menu strip across the top: rendering, hit-test, mnemonic handling (`F` / `E` / `V`), and dispatch back to `GameManager` for the actual file / edit / view actions. Menu logic stays here, not in `GameManager`. When a menu item is clicked, the bar calls a callback registered by `GameManager` — it does not reach into other subsystems.
